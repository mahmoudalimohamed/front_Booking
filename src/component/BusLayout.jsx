...
const BusLayout = ({ availableSeats, chosenSeats, unavailableSeats, toggleSeatSelection, loading }) => {
  ...
  if (availableSeats.length === 0) {return (<p className="text-gray-600 mb-4">No seats available for this trip.</p>);}
  ...
  const maxSeatNumber = Math.max(...availableSeats.map((seat) => parseInt(seat.seat_number)));
              ...
              <div className="flex gap-2">
                {[1, 2].map((seatPosition) => {
                  ...
                  if (seatNumber <= maxSeatNumber) {
                    const seatExists = availableSeats.some( (seat) => parseInt(seat.seat_number) === seatNumber);

                    if (!seatExists) return ( <div key={seatNumber} className={`${seatSize} invisible`}> </div>);

                    return (
                      <button key={seatNumber}
